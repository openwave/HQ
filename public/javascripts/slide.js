	...
	return{
		init:function(){
			...
			for(i=0;i<len;i++){
				var id=t[i].value; 
				tar[i]=id;
				t[i].onclick=new Function("slideShow.getimgset('"+id+"')");
				if(i==0){
					this.getimg(id)
				}
			}
		...
		nav:function(d){
			...
			if(tar[parseInt(c)+d]){
				...
				if(d==1){
					...
					this.getimg(tar[tar.length-1])
				}
			}
		},
		
		auto:function(){ia.timer=setInterval(function(){slideShow.nav(1)},autodelay*5000)},
		fdin:function(i){
		...
		initPanels:function(target_items, name){
		 	jQuery(target_items).each(
				function(i){
					if (name == "mainPanel"){
						jQuery(this).prepend(
							"<span class='" + name + "' id='" + name + i + "'><div class='grid_4' style='padding-top:5px'>Content Here</div><div class='grid_2' style='float:right; text-align:right;'><img src='/images/removeWhite.jpg' height='25' /></div></span>");
					} else {
						jQuery(this).prepend("<span class='" + name + "' id='" + name + i + "'>" + name + i + "</span>");
						jQuery(this).append("<span class='" + name + "Bottom" + "' id='" + "B" + name + i + "'><img src='/images/moveLeft.gif' height='20' /><img src='/images/moveRight.gif' height='20' /> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <img src='/images/removeWhite.png' height='20' /></span>");
					}
					var my_tooltip = jQuery("#"+name+i);
					var my_tooltipBottom = jQuery("#B"+name+i);
					jQuery(this).hover( 
						function(event) {
							my_tooltip.css({display:"inline"}).fadeIn(400);
							my_tooltipBottom.css({display:"inline"}).fadeIn(400);
						},
						function(event) {
							my_tooltip.fadeOut(400);
							my_tooltipBottom.fadeOut(400);
						} 
					);			
				}
			);
...
jQuery(document).ready(function(){ 
	slideShow.init(); 
	slideShow.lim();
	slideShow.initPanels("div.frame", "thumbPanel");
	//slideShow.initPanels("div.mainFrame", "mainPanel");
});